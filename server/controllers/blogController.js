...
const getBlogs = async (req, res) => {
  try {
    ...
    if (!blogs.success) return res.status(401).json(blogs);
...
const getBlog = async (req, res) => {
  try {
    ...
    if (!blog.success) return res.status(401).json(blog);
    return res.status(201).json(blog);
  } catch (error) {
    return res.status(501).json({
      success: false,
...
const removeBlog = async (req, res) => {
  ...
  try {
    ...
    if (!respsonse.success) return res.status(401).json(respsonse);
